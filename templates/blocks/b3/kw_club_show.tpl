...
<link href="<{$xoops_url}>/modules/Kw_device/assets/css/module.css" media="all" rel="stylesheet">
    ...
    <div class="row">
        <div class="col-sm-10">
            ...
            <h4>
                ...
                <{if !$block.chk_time}>
                    <span class="label label-danger"><{$smarty.const._MB_KWCLUB_NON_REGISTRATION_TIME}></span>
                <{/if}>
            </h4>
        </div>
        <div class="col-sm-2" style="padding-top: 40px;">
            <{if $iskwDeviceAdmin || $smarty.session.isclubUser}>
                <a href="<{$xoops_url}>/modules/Kw_device/club.php?club_year=<{$block.club_year}>" class="btn btn-primary btn-block" <{if !$block.can_operate}>data-toggle="tooltip" data-placement="bottom" title="<{$smarty.const._MB_KWCLUB_OVER_END_TIME}>" disabled<{/if}>><i class="fa fa-plus" aria-hidden="true"></i>
                ...
                <ul id="tr_<{$block.data.class_id}>">
                    ...
                    <li class="vm w2">
                        ...
                        <{if $iskwDeviceAdmin}>
                            <{$data.class_isopen}>
                        <{/if}>
                        <a href="<{$xoops_url}>/modules/Kw_device/index.php?class_id=<{$data.class_id}>"><{$data.class_title}></a>
                        <div style="font-size: 0.9em;">
                            ...
                            <a href="<{$xoops_url}>/modules/Kw_device/index.php?op=teacher#<{$data.teacher_id}>"><{$data.teacher_id_title}></a>
                    ...
                    <li class="vm w2">
                        ...
                        <div>
                            <!--上課星期-->
                            <{if $data.class_week==_MD_KWDEVICE_ALL_WEEK}>
                    ...
                    <li class="vm w1 text-center">
                        <{if $iskwDeviceAdmin}>
                            <{if $data.class_ischecked!=1}>
                                <a href="<{$xoops_url}>/modules/Kw_device/club.php?op=class_enable&class_id=<{$data.class_id}>" class="btn btn-xs btn-success" data-toggle="tooltip" data-placement="bottom" title="<{$smarty.const._MB_KWCLUB_CLASS_ENABLE_DESC}>"><{$smarty.const._MB_KWCLUB_CLASS_ENABLE}></a>
                            <{else}>
                                <a href="<{$xoops_url}>/modules/Kw_device/club.php?op=class_unable&class_id=<{$data.class_id}>" class="btn btn-xs btn-danger" data-toggle="tooltip" data-placement="bottom" title="<{$smarty.const._MB_KWCLUB_CLASS_UNABLE_DESC}>"><{$smarty.const._MB_KWCLUB_CLASS_UNABLE}></a>
                            <{/if}>
                            <a href="<{$xoops_url}>/modules/Kw_device/club.php?op=class_blank&class_id=<{$data.class_id}>" class="btn btn-xs btn-default" data-toggle="tooltip" data-placement="bottom" title="<{$smarty.const._MB_KWCLUB_CLASS_BLANK_DESC}>"><{$smarty.const._MB_KWCLUB_CLASS_BLANK}></a>
                        <{/if}>

                        <{if $iskwDeviceAdmin || $block.uid == $data.class_uid}>
                            <a href="<{$xoops_url}>/modules/Kw_device/club.php?class_id=<{$data.class_id}>" class="btn btn-xs btn-warning" <{if !$block.can_operate}>data-toggle="tooltip" data-placement="bottom" title="<{$smarty.const._MB_KWCLUB_OVER_END_TIME}>" disabled<{/if}>><{$smarty.const._TAD_EDIT}></a>
                            <{if $data.class_regnum == 0}>
                                <a href="javascript:delete_class_func(<{$data.class_id}>);" class="btn btn-xs btn-danger" <{if !$block.can_operate}>data-toggle="tooltip" data-placement="bottom" title="<{$smarty.const._MB_KWCLUB_OVER_END_TIME}>" disabled<{/if}>><{$smarty.const._TAD_DEL}></a>
                            <{/if}>
                        <{else}>
                            <{if $data.is_full}>
                                <a href="#" class="btn btn-danger btn-xs disabled" data-toggle="tooltip" data-placement="bottom" title="<{$smarty.const._MB_KWCLUB_NUMBER_OF_APPLICANTS}> <{$data.class_regnum}> <{$smarty.const._MB_KWCLUB_PEOPLE}>"><i class="fa fa-user-plus" aria-hidden="true"></i>
                                    <{$smarty.const._MB_KWCLUB_FULL_REGISTRATION}></a>
                            <{elseif $data.class_regnum >= $data.class_member}>
                                <a href="<{$xoops_url}>/modules/Kw_device/index.php?op=reg_form&class_id=<{$data.class_id}>&is_full=1" class="btn btn-warning btn-xs"><i class="fa fa-user-plus" aria-hidden="true"></i>
                                    <{$smarty.const._MB_KWCLUB_SIGNUP_TO_MAKE_UP}></a>
                            <{elseif $block.chk_time}>
                                <a href="<{$xoops_url}>/modules/Kw_device/index.php?op=reg_form&class_id=<{$data.class_id}>" class="btn btn-primary btn-xs"><i class="fa fa-user-plus" aria-hidden="true"></i>
